module Vector.MouseTracingNormalized exposing (..)
...
normalize magnitude position =
  Vector2.add center <| Vector2.scale magnitude <| Vector2.normalize <| Vector2.sub position center
...
arrow position =
  let
    { x, y } =
      Vector2.toRecord <| normalize 100 position
  in
  Svg.line
    [ Attributes.x1 (px <| x)
    , Attributes.y1 (px <| y)
