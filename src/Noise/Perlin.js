...
var $author$project$Noise$Perlin$GetPerlin = function (a) {
	return {$: 'GetPerlin', a: a};
};
...
var $author$project$Noise$Perlin$getPerlin = _Platform_incomingPort('getPerlin', $elm$json$Json$Decode$float);
	...
	return $elm$core$Platform$Sub$batch(
		_List_fromArray(
			[
				...
				$author$project$Noise$Perlin$getPerlin($author$project$Noise$Perlin$GetPerlin)
			]));
};
...
var $author$project$Noise$Perlin$lerp = F5(
	function (min1, max1, min2, max2, num) {
		var ratio = $elm$core$Basics$abs(num / (max1 - min1));
		return min2 + (ratio * (max2 - min2));
	});
var $elm$json$Json$Encode$float = _Json_wrap;
var $author$project$Noise$Perlin$perlin1d = _Platform_outgoingPort('perlin1d', $elm$json$Json$Encode$float);
		...
		if (msg.$ === 'GetPerlin') {
				...
				_Utils_update(
					model,
					{
						lengths: A2(
							...
							_Utils_Tuple2(
								model.time,
								A5($author$project$Noise$Perlin$lerp, 0, 1, 0, 600, n)),
							model.lengths)
					}),
				$elm$core$Platform$Cmd$none);
		} else {
			...
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{time: newTime}),
				$author$project$Noise$Perlin$perlin1d(newTime));
		}
	});
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				$elm_community$typed_svg$TypedSvg$Types$px(time)),
...
var $author$project$Noise$Perlin$main = $elm$browser$Browser$element(
	{init: $author$project$Noise$Perlin$init, subscriptions: $author$project$Noise$Perlin$subscriptions, update: $author$project$Noise$Perlin$update, view: $author$project$Noise$Perlin$view});
_Platform_export({'Noise':{'Perlin':{'init':$author$project$Noise$Perlin$main(
