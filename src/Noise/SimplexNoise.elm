...
generatePermMod12 perm =
  Array.map (\i -> remainderBy 12 i) perm
...
getN4d x y z w i j k l perm permMod12 =
  ...
  if t < 0 then
    ...
    let
      gi =
        remainderBy 32 (get perm i + (get perm j + (get perm k + get perm l))) * 4
  ...
  let
    ( perm, seed_1 ) =
      Random.step permGenerattor seed
        |> (\( list, seed_2 ) -> ( Array.append list (reverseArray list), seed_2 ))
  in
  ( { perm = perm, permMod12 = generatePermMod12 perm }, seed_1 )


{-| Generates a noise value between `-1` and `1` based on the given x value and a seeded permutation table.
...
noise1d permTable xin =
  noise2d permTable xin 0
