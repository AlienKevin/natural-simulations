module Forces.BlowingWindWithGravityAndFriction exposing (..)
  ...
  Random.generate GetBalls <|
    Random.list
      ...
      ( Random.map2
        (\mass x ->
          { mass = mass
          , position = Vector2.vec2 x (height / 2)
...
moveBall b =
  let
    ...
    windForce =
      Vector2.vec2 0.01 0
    gravityForce =
      Vector2.vec2 0 (0.1 * b.mass)
    frictionForce =
      Vector2.scale -0.01 <| normalize b.velocity
    newAcceleration =
      applyForce windForce b.mass <|
        applyForce gravityForce b.mass <|
          applyForce frictionForce b.mass <|
            Vector2.vec2 0 0
    newVelocity =
