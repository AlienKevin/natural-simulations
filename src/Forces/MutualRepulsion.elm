...
updateMover mover others =
  let
    newAcceleration =
      List.foldl
        (\repulsor acceleration->
          let
            repulsionForce =
              calculateRepulsion mover repulsor
          in
          applyForce repulsionForce repulsor.mass acceleration
        )
        (Vector2.vec2 0 0)
        others
    newVelocity =
...
calculateRepulsion : Mover -> Mover -> Vector2.Vec2
calculateRepulsion mover repulsor  =
  let
    repulsionDistance =
      limit 5 25 <| Vector2.sub repulsor.position mover.position
    repulsionDir =
      Vector2.normalize <| repulsionDistance
    repulsionDistanceSquared =
      Vector2.lengthSquared repulsionDistance
    repulsionForce =
      Vector2.scale ( -1 * 5 * repulsor.mass * repulsor.mass / repulsionDistanceSquared ) repulsionDir 
  in
  repulsionForce
